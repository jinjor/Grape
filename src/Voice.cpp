    ...
    for(int i = 0; i < NUM_LFO; ++i) {
        ...
        switch(targetType) {
            case LFO_TARGET_TYPE::OSC: {
                ...
                switch(param) {
                    case LFO_TARGET_OSC_PARAM::Vibrato: {
                        ...
                        auto v = lfoValue * lfoAmount * by12;
                        if (targetIndex == NUM_OSC) {
                            for(int oscIndex = 0; oscIndex < NUM_OSC; ++oscIndex) {
                                modifiers.octShift[oscIndex] += v;
                            }
                    ...
                    case LFO_TARGET_OSC_PARAM::FM: {
                        auto v = lfoValue * lfoAmount * juce::MathConstants<double>::twoPi;
                        if (targetIndex == NUM_OSC) {
                            for(int oscIndex = 0; oscIndex < NUM_OSC; ++oscIndex) {
                                modifiers.angleShift[oscIndex] += v;
                            }
                        } else {
                          modifiers.angleShift[targetIndex] += v;
                        }
                        break;
                    }
            ...
            case LFO_TARGET_TYPE::Filter: {
                int targetIndex = params.TargetFilter->getIndex();
                auto param = static_cast<LFO_TARGET_FILTER_PARAM>(
                params.TargetFilterParam->getIndex());
                switch (param) {
